...
def collect_results(m,controlled_layers,T,N_hat,N_hat_prime,A_hat_prime,decision_vars,S):
    ...
    for t in range(T):
        ...
        for n,d in N_hat_prime:
            ...
            if T == 1:
                nodeVar='w_'+`n`+","+`t`
            if round(m.getVarByName(nodeVar).x)==1 or decision_vars[t]['w_'+`n`]==1:
                action=`n[0]`+"."+`n[1]`
        ...
        for u,v,a in A_hat_prime:
            ...
            if T == 1:
                arcVar='y_'+`u`+","+`v`+","+`t`
            if round(m.getVarByName(arcVar).x)==1 or decision_vars[t]['y_'+`u`+","+`v`]==1:
        ...
        for u,v,a in A_hat_prime:
            ...
            if m.getVarByName(arcVar).x==0 and decision_vars[t]['y_'+`u`+","+`v`]==1:
        ...
        for n,d in N_hat_prime:
            ...
            if m.getVarByName(nodeVar).x==0 and decision_vars[t]['w_'+`n`]==1:
        ...
        for n,d in N_hat.nodes_iter(data=True):
            ...
            overSuppCost_layer[n[1]]+= d['data']['inf_data'].oversupply_penalty*m.getVarByName('delta+_'+`n`+","+`t`).x
            underSupp+= m.getVarByName('delta+_'+`n`+","+`t`).x
            underSupp_layer[n[1]]+= m.getVarByName('delta+_'+`n`+","+`t`).x
            underSuppCost+=d['data']['inf_data'].undersupply_penalty*m.getVarByName('delta-_'+`n`+","+`t`).x
            underSuppCost_layer[n[1]]+=d['data']['inf_data'].undersupply_penalty*m.getVarByName('delta-_'+`n`+","+`t`).x
        ...
        for u,v,a in N_hat.edges_iter(data=True):
            ...
            flowCost_layer[u[1]]+=a['data']['inf_data'].flow_cost*m.getVarByName('x_'+`u`+","+`v`+","+`t`).x
