...
import time
import sys
def run_judgment_call(params, layers, T=1, saveJC=True, print_cmd=True, saveJCModel=False):
    """ Solves an INDP problem with specified parameters using a decentralized hueristic
    called Judgment Call . Outputs to directory specified in params['OUTPUT_DIR'].
    ...
    InterdepNet = None
    if "N" not in params:
        InterdepNet = indp.initialize_network(BASE_DIR="../data/INDP_7-20-2015/", sim_number=params['SIM_NUMBER'], magnitude=params["MAGNITUDE"])
    else:
    ...
    if T == 1:
        ...
        indp_results_initial = indp.indp(InterdepNet, 0, 1, layers, controlled_layers=layers)
        ...
        for i in range(num_iterations):
            ...
            if auction_type:
                res_allocate[i], PoA[i], valuations[i], auction_time, valuation_time = auction_resources(sum(v_r), params,
                            layers=layers, print_cmd=print_cmd, judgment_type=judgment_type,
            ...
            for P in layers:
                ...
                indp_results = indp.indp(InterdepNet, v_r_applied[P-1], 1, layers=layers,
                ...
                indp.apply_recovery(InterdepNet, Dindp_results[P], i+1)
                Dindp_results[P].add_components(i+1, indputils.INDPComponents.calculate_components(indp_results[0], InterdepNet, layers=[P]))
            ...
            for P in layers:
                if print_cmd:
                    print( "Layer-%d"%(P) )
                indp_results_Real, realizations = Decentralized_INDP_Realized_Performance(InterdepNet,
                ...
                if saveJCModel:
                    indp.save_INDP_model_to_file(indp_results_Real[0], output_dir+"/Model", i+1, P, suffix='Real')
        ...
        print('hahahaha')
def Decentralized_INDP_Realized_Performance(N, indp_results, functionality, layers,
                                            T=1, controlled_layers=[1], print_cmd=False):
    ...
    actually decides (as opposed to according to the guess). For output items, look at
    the description of Decentralized_INDP()"""
    ...
    for t in range(T):
        ...
        for u, _ in functionality[t].items():
            if u in list_interdep_nodes:
                realCount += 1
                vValues = [G_prime.nodes[x]['data']['inf_data'].functionality for x in interdep_nodes[u]]
            ...
            for n in G_prime[P].nodes(data=True):
                if not n[0] in prob.keys():
                    ...
                    for ll in com[P]:
                        ...
                        if value>0:
                            p.append(value/maxValues[P, ll, 'Supply'])
                        elif value<=0:
                            p.append(value/maxValues[P, ll, 'Demand'])
                        else:
                            print( 'Are you kidding me???')
                    prob[n[0]] = [max(p), np.random.choice([0, 1],  p=[1-max(p),  max(p)])]
    return prob
def auction_resources(v_r, params, layers, print_cmd=True, judgment_type="OPTIMISTIC", auction_type="MDA", valuation_type='DTC_uniform'):
...
#                resource_allocation[winner].append(v+1)
    ...
    if auction_type=="MCA":
        ...
        m.update()
        # Add constraints
        numAllocatedResources = gurobipy.LinExpr()
        for P in layers:
            eachBidderAllocation = gurobipy.LinExpr()
            for v in range(v_r):
                numAllocatedResources+=m.getVarByName('y_'+str(v+1)+", "+str(P))*(v+1)
                eachBidderAllocation+=m.getVarByName('y_'+str(v+1)+", "+str(P))
            m.addConstr(eachBidderAllocation, gurobipy.GRB.LESS_EQUAL, 1.0, "Bidder "+str(P)+" allocation")
        m.addConstr(numAllocatedResources, gurobipy.GRB.LESS_EQUAL, v_r, "Total number of resources")
        #    print "Solving..."
        m.update()
        m.optimize()
        for P in layers:
...
def compute_valuations(v_r, InterdepNet, layers, T=1, print_cmd=True, judgment_type="OPTIMISTIC", valuation_type='DTC_uniform', compute_optimal_valuation=False):
    ...
    for P in layers:
        ...
        indp_results = indp.indp(InterdepNet, v_r=0, T=1, layers=layers,
                            controlled_layers=[P])
    ...
    if compute_optimal_valuation:
        ...
        indp_results = indp.indp(InterdepNet, v_r=v_r, T=1, layers=layers,
                                    controlled_layers=layers)
            ...
            if valuation_type=='DTC':
                for v in range(v_r):
                    ...
                    functionality = create_judgment_matrix(InterdepNet, T, negP, v_r,
                                                           judgment_type=judgment_type)
                    ###!!! check what v_r must be for det demand JC###
                    indp_results = indp.indp(InterdepNet, v_r=v+1,
        ...
        for t, value in res_allocate.items():
            ...
            if valuations:
                for titem in res_alloc_time[t][2]:
                    row += str(titem)+', '
            f.write(row+"\n")
def read_resourcec_allocation(df, combinations, optimal_combinations, ref_method='indp',
...
def write_judgments_csv(outdir, realizations, sample_num=1, agent=1, time_step=0, suffix=""):
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    judge_file = outdir+'/judge_'+str(sample_num)+"_agent"+str(agent)+'_time'+str(time_step)+'_'+suffix+".csv"
...
def correct_tdindp_results(df, optimal_combinations):
    ...
    return df
def relative_performance(df, combinations, optimal_combinations, ref_method='indp',
...
def generate_combinations(database, mags, sample, layers, no_resources, decision_type, 
    ...
    if database=='shelby':
        if list_high_dam_add:
            listHD = pd.read_csv(list_high_dam_add)     
        L = len(layers)
              
        for m, s in itertools.product(mags, sample):
            if list_high_dam_add==None or len(listHD.loc[(listHD.set == s) & (listHD.sce == m)].index):
