    ...
    for key, val in train_data.items():
        if key[-2]==`2`:
            with pm.Model() as logistic_model[key]:
                ...
                trace[key] = pm.sample(1000, tune=500,chains=2, cores=2)
...
def test_model(train_data_all,train_data,trace_all,model_all,key):
...
#    plt.rc('font', **{'family': 'serif', 'serif': ['Computer Modern']})
    ...
    test_data=train_data[key].iloc[[random.randint(0, train_data[key].shape[1]) for p in range(0, 100)],:]
