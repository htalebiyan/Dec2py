...
if __name__ == "__main__":  
    ...
    failSce_param = {"type":"random","sample_range":range(50,51),"mags":range(0,1),
        ...
        for s in range(no_samples):
            for t in range(no_time_steps):
                ...
                for key,val in samples.iteritems():
                    decision_vars[0][key]=val[t,s] #0 becasue iINDP
                flow_results=indp.flow_problem(initial_net,v_r=4,layers=layers,controlled_layers=layers,
                              decision_vars=decision_vars,print_cmd=True, time_limit=None)
                # flow_results=flow.flow_problem(initial_net,v_r=100,layers=layers,controlled_layers=layers,
                #              decision_vars=decision_vars,print_cmd=True, time_limit=None)
                ...
                real_results=real_results.from_csv(folder_name,sample_num=failSce_param['sample_range'][0]+s,suffix="")
