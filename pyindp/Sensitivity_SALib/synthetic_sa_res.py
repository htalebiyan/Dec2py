...
#     'bounds': [[2, 4], [10, 50], [2, 3], gamma_dict[topo], [0.05, 0.5], [2, 400]]
# }
...
# Y = pd.concat([comp_res['distance_to_optimal']], axis=1, keys=['omega']).to_numpy().ravel()
# delta = delta.analyze(problem, X, Y, num_resamples=500, conf_level=0.95, print_to_console=True, seed=None)
...
'''Plot results'''
results = pd.read_csv('Results_res.csv', header=0)
