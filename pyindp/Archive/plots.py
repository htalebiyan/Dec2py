...
def plot_relative_performance_shelby(lambda_df,cost_type='Total'):    
        ...
        for idxvt,vt in enumerate(valuation_type): 
            ...
            with sns.color_palette("RdYlGn", 8):  #sns.color_palette("YlOrRd", 7)
                ...
                if idxvt!=len(valuation_type)-1:
                    ax.set_xlabel('')
                ax.set_ylabel(r'E[$\lambda_{%s}$], Valuation: %s'%('TC',valuation_type[idxvt]))
                if idxnr!=0:
                    ax.set_ylabel('')
...
def plot_relative_performance_synthetic(lambda_df,cost_type='Total'):     
    ...
    valuation_type = lambda_df.valuation_type.unique().tolist()
    valuation_type.remove('')
    inter_type  = lambda_df.interdependency.unique().tolist()
    ...
    for idxnr,nr in enumerate(hor_grid):
        for idxvt,vt in enumerate(ver_grid):
            ...
            with sns.color_palette("RdYlGn", 8):  #sns.color_palette("YlOrRd", 7)
                ...
                if idxvt!=len(valuation_type)-1:
                    ax.set_xlabel('')
                ax.set_ylabel(r'E[$\lambda_{%s}$], Valuation Type = %s'%('TC',vt))
