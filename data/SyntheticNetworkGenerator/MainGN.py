...
# Input values
noSamples = 10 # Number of sample sets of network
...
for cnfg in range(0,noConfiguration):
    ...
    gridsizeXDict={x:int(round(gridsizeX*(1+np.random.normal(0, paramError)))) for x in range(noLayers)}
    gridsizeY = np.random.randint(low=3, high=10)
    while gridsizeY>50.0/gridsizeX:
        gridsizeY = np.random.randint(low=5, high=10)
    gridsizeYDict={x:int(round(gridsizeY*(1+np.random.normal(0, paramError)))) for x in range(noLayers)}
    noNodesDict={x:int(round(gridsizeXDict[x]*gridsizeYDict[x])) for x in range(noLayers)}
