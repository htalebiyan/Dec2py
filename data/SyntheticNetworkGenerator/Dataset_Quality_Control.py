...
def load_array_format_extended(
        BASE_DIR="C:\\Users\\ht20\Documents\\Files\Generated_Network_Dataset_v3\\RandomNetworks\\",
            ...
            if file == 'N' + str(k) + '_Nodes.txt':
                with open(file_dir + file) as f:
                    ...
                    for v in data.iterrows():
                        ...
                        G.nodes[node_name]['data'] = node_data
...
def plot_network(BASE_DIR, topo, config=0, sample=0, cost_scale=1.0):
    plt.close('all')
...
def initialPerformanceUndamagedNetwork(G, noLayers):
    ...
    subarcs = {x: [(a[0][0], a[1][0]) for a in arcs if a[0][1] == x and a[1][1] == x] for x in
               nets}  # Subset of arcs that belong to each network

    # Obj. Costs				# Cost of fixing node i.j of network k
    M_p = {(i, k, l): G.nodes(data=True)[(i, k)]['data']['Mp'] for k in nets for l in commodities for i in subnodes[k]
           if l in com[k]}  # Penalty for excess
    M_m = {(i, k, l): G.nodes(data=True)[(i, k)]['data']['Mm'] for k in nets for l in commodities for i in subnodes[k]
           if l in com[k]}  # Penalty for shortage
    c = {(i, j, k, l): G[(i, k)][(j, k)]['data']['c'] for k in nets for l in commodities for (i, j) in subarcs[k] if
         l in com[k]}  # Flow cost

    # Coefs & Right-hand Sid
    # Demands at nodes (fictional)
    b = {(i, k, l): G.nodes(data=True)[(i, k)]['data']['b'] for k in nets for l in commodities for i in subnodes[k] if
         l in com[k]}
    # arc capacities
    u = {(i, j, k, l): G[(i, k)][(j, k)]['data']['u'] for k in nets for l in commodities for (i, j) in subarcs[k] if
         l in com[k]}  # Capacity of arc i,j in network k

    # gamma: is it necessary that node i (net k) k is functional for node j (net ka) to work?
    gamma = {(i, j, k, ka): 0 for k in nets for ka in nets for i in nodes for j in nodes if i in subnodes[k] if
             j in subnodes[ka] if i != j if k != ka}
...
rootfolder = "C:/Users/ht20/Documents/Files/Generated_Network_Dataset_v4/"
...
#             layer_da = len([x for x in dam_arcs if x[0][1] == k + 1 and x[1][1] == k + 1]) / float(
#                 len(node_list)) / float(noLayers) / 2.0
...
''' Plot one network '''
plot_network(BASE_DIR=rootfolder, topo=NetworkTypeInitial, config=12, sample=0)
