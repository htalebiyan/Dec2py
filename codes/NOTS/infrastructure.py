...
def load_infrastructure_data(BASE_DIR="../data/INDP_7-20-2015/", external_interdependency_dir=None,
    ...
    if data_format == 'shelby_old':
        ...
        G = load_infrastructure_array_format(BASE_DIR=BASE_DIR,
                                             ...
                                             magnitude=magnitude, v=v, sim_number=sim_number, cost_scale=cost_scale)
        #        print G #!!!
        return G
        ...
        with open(BASE_DIR + file) as f:
            ...
            for v in data.iterrows():
                if fname == 'beta':
                    ...
                    G.G[(int(v[1]['Start Node']), net)][(int(v[1]['End Node']), net)]['data']['inf_data'].space = int(
                        int(v[1]['Subspace']))
                    G.G[(int(v[1]['End Node']), net)][(int(v[1]['Start Node']), net)]['data']['inf_data'].space = int(
                        int(v[1]['Subspace']))
                if fname == 'alpha':
                    net = netNames[v[1]['Network']]
