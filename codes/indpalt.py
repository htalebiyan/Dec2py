...
def flow_problem(N, v_r, T=1, layers=[1, 3], controlled_layers=[1, 3], decision_vars={}, functionality={},
                 forced_actions=False, print_cmd=True, time_limit=None):
    ...
    for t in range(T):
        for n, d in n_hat.nodes(data=True):
            if n not in interdep_nodes.keys() or decision_vars[t]['w_' + str(
                    n)] == 0:  # Beacuse repaired interdpendent nodes may beforced to become non-functional by the dependee node
                m.getVarByName('w_' + str(n) + "," + str(t)).lb = decision_vars[t]['w_' + str(n)]
                m.getVarByName('w_' + str(n) + "," + str(t)).ub = decision_vars[t]['w_' + str(n)]
        for u, v, a in a_hat_prime:
            m.getVarByName('y_' + str(u) + "," + str(v) + "," + str(t)).lb = decision_vars[t][
                'y_' + str(u) + "," + str(v)]
            m.getVarByName('y_' + str(u) + "," + str(v) + "," + str(t)).ub = decision_vars[t][
                'y_' + str(u) + "," + str(v)]
    ...
    for t in range(T):
        ...
        for u, v, a in a_hat_prime:
            if T == 1:
                objFunc += (float(a['data']['inf_data'].reconstruction_cost) / 2.0) * m.getVarByName(
        ...
        for n, d in n_hat_prime:
            if T == 1:
                ...
                objFunc += d['data']['inf_data'].reconstruction_cost * m.getVarByName(
    ...
    for t in range(T):
        ...
        for n, d in n_hat.nodes(data=True):
            if not functionality:
                if n in interdep_nodes:
                    ...
                    m.addConstr(interdepLConstr, GRB.GREATER_EQUAL, interdepRConstr,
        ...
        for s in S:
            for n, d in n_hat_prime:
                if T == 1:
                    m.addConstr(m.getVarByName('w_' + str(n) + "," + str(t)) * d['data']['inf_data'].in_space(s.id),
            ...
            for u, v, a in a_hat_prime:
                if T == 1:
                    m.addConstr(
                        m.getVarByName('y_' + str(u) + "," + str(v) + "," + str(t)) * a['data']['inf_data'].in_space(
...
def collect_results(m, controlled_layers, T, N_hat, N_hat_prime, A_hat_prime, S):
    ...
    for t in range(T):
        ...
        for n, d in N_hat.nodes(data=True):
            overSuppCost += d['data']['inf_data'].oversupply_penalty * m.getVarByName(
                'delta+_' + str(n) + "," + str(t)).x
            overSuppCost_layer[n[1]] += d['data']['inf_data'].oversupply_penalty * m.getVarByName(
                'delta+_' + str(n) + "," + str(t)).x
            ...
            underSuppCost += d['data']['inf_data'].undersupply_penalty * m.getVarByName(
                'delta-_' + str(n) + "," + str(t)).x
            underSuppCost_layer[n[1]] += d['data']['inf_data'].undersupply_penalty * m.getVarByName(
                'delta-_' + str(n) + "," + str(t)).x
        ...
        for u, v, a in N_hat.edges(data=True):
            ...
            flowCost_layer[u[1]] += a['data']['inf_data'].flow_cost * m.getVarByName(
...
def indp_relax(N, v_r, T=1, layers=[1, 3], controlled_layers=[1, 3], functionality={}, forced_actions=False,
               print_cmd=True, time_limit=None):
    """Relaxed INDP optimization problem. Also solves td-INDP if T > 1.
    :param N: An InfrastructureNetwork instance (created in infrastructure.py)
    :param v_r: Vector of number of resources given to each layer in each timestep. If the size of the vector is 1, it shows the total number of resources for all layers.
    :param T: Number of timesteps to optimize over.
    :param layers: Layer IDs of N included in the optimization. (Default is water (1) and power (3)).
    ...
    for t in range(T):
        # Add geographical space variables.
        for s in S:
            m.addVar(name='z_' + str(s.id) + "," + str(t), lb=0.0, ub=1.0)
        # Add over/undersupply variables for each node.
        for n, d in N_hat.nodes(data=True):
            m.addVar(name='delta+_' + str(n) + "," + str(t), lb=0.0)
            m.addVar(name='delta-_' + str(n) + "," + str(t), lb=0.0)
        # Add functionality binary variables for each node in N'.
        for n, d in N_hat.nodes(data=True):
            m.addVar(name='w_' + str(n) + "," + str(t), lb=0.0, ub=1.0)
            if T > 1:
                m.addVar(name='w_tilde_' + str(n) + "," + str(t), lb=0.0, ub=1.0)
        # Add flow variables for each arc.
        for u, v, a in N_hat.edges(data=True):
            m.addVar(name='x_' + str(u) + "," + str(v) + "," + str(t), lb=0.0)
        # Add functionality binary variables for each arc in A'.
        for u, v, a in A_hat_prime:
            m.addVar(name='y_' + str(u) + "," + str(v) + "," + str(t), lb=0.0, ub=1.0)
            if T > 1:
                m.addVar(name='y_tilde_' + str(u) + "," + str(v) + "," + str(t), lb=0.0, ub=1.0)
    m.update()
    ...
    for t in range(T):
        ...
        for u, v, a in A_hat_prime:
            indexLayer = a['data']['inf_data'].layer - 1
            if T == 1:
                resourceLeftConstr += 0.5 * a['data']['inf_data'].resource_usage * m.getVarByName(
                    'y_' + str(u) + "," + str(v) + "," + str(t))
                if isSepResource:
                    resourceLeftConstrSep[indexLayer] += 0.5 * a['data']['inf_data'].resource_usage * m.getVarByName(
                        'y_' + str(u) + "," + str(v) + "," + str(t))
            else:
                resourceLeftConstr += 0.5 * a['data']['inf_data'].resource_usage * m.getVarByName(
                    'y_tilde_' + str(u) + "," + str(v) + "," + str(t))
        ...
        for n, d in N_hat_prime:
            indexLayer = n[1] - 1
            if T == 1:
                resourceLeftConstr += d['data']['inf_data'].resource_usage * m.getVarByName(
                    'w_' + str(n) + "," + str(t))
                if isSepResource:
                    resourceLeftConstrSep[indexLayer] += d['data']['inf_data'].resource_usage * m.getVarByName(
                        'w_' + str(n) + "," + str(t))
            else:
                resourceLeftConstr += d['data']['inf_data'].resource_usage * m.getVarByName(
                    'w_tilde_' + str(n) + "," + str(t))
        ...
        if isSepResource:
            for k in range(len(v_r)):
                m.addConstr(resourceLeftConstrSep[k], GRB.LESS_EQUAL, v_r[k],
...
def collect_results_relax(m, controlled_layers, T, N_hat, N_hat_prime, A_hat_prime, S):
    layers = controlled_layers
    indp_results = INDPResults(layers)
    # compute total demand of all layers and each layer
    total_demand = 0.0
    ...
    for t in range(T):
        ...
        # Calculate space preparation costs.
        for s in S:
            spacePrepCost += s.cost * round(m.getVarByName('z_' + str(s.id) + "," + str(t)).x)
        indp_results.add_cost(t, "Space Prep", spacePrepCost, spacePrepCost_layer)
        # Calculate arc preparation costs.
        for u, v, a in A_hat_prime:
            arcVar = 'y_tilde_' + str(u) + "," + str(v) + "," + str(t)
            if T == 1:
                arcVar = 'y_' + str(u) + "," + str(v) + "," + str(t)
            arcCost += (a['data']['inf_data'].reconstruction_cost / 2.0) * round(m.getVarByName(arcVar).x)
            arcCost_layer[u[1]] += (a['data']['inf_data'].reconstruction_cost / 2.0) * round(m.getVarByName(arcVar).x)
        indp_results.add_cost(t, "Arc", arcCost, arcCost_layer)
        # Calculate node preparation costs.
        for n, d in N_hat_prime:
            nodeVar = 'w_tilde_' + str(n) + "," + str(t)
            if T == 1:
                nodeVar = 'w_' + str(n) + "," + str(t)
            nodeCost += d['data']['inf_data'].reconstruction_cost * round(m.getVarByName(nodeVar).x)
            nodeCost_layer[n[1]] += d['data']['inf_data'].reconstruction_cost * round(m.getVarByName(nodeVar).x)
        indp_results.add_cost(t, "Node", nodeCost, nodeCost_layer)
        # Calculate under/oversupply costs.
        for n, d in N_hat.nodes(data=True):
            overSuppCost += d['data']['inf_data'].oversupply_penalty * m.getVarByName(
