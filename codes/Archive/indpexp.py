...
def run_percolation_model():
    ...
    outfile=None
    loadfile="../../percisland/networks/power_grid_"+str(sample)+"_N"+str(N)+"_loads.csv"
    ...
    params={"NUM_ITERATIONS":len(supply_net.N)*3,"OUTPUT_DIR":'../results/percmodel_T'+str(T)+'_N'+str(len(supply_net.N))+"_S"+str(p_S)+"_C"+str(p_C)+"_"+demand_distr,"ALGORITHM":'INDP',"V":1}
    if T > 1:
...
def run_backwards_induction_L2_V2(mags):
    for m in mags:
        params={"T":20,"OUTPUT_DIR":"../results/bi_results_L2_3-1_m"+str(m)+"_v1","MAGNITUDE":m,"V":1,"ALGORITHM":"BACKWARDS_INDUCTION"}
        batch_run(params,layers=[1,3],player_ordering=[3,1])
...
def run_inrg_L2_V2_random_flip1(mags):
    ...
    for m in mags:
        params = {}
        if m == 8:
            params={"NUM_ITERATIONS":50,"OUTPUT_DIR":'../results/inrg_flip_'+str(num_flips)+'_results_L2',"MAGNITUDE":m,"V":1,"T":1,"ALGORITHM":"INRG","SIM_NUMBER":110,"NUM_FLIPS":num_flips}
        else:
            params={"NUM_ITERATIONS":50,"OUTPUT_DIR":'../results/inrg_flip_'+str(num_flips)+'_results_L2',"MAGNITUDE":m,"V":1,"T":1,"ALGORITHM":"INRG","SIM_NUMBER":446,"NUM_FLIPS":num_flips}
...
def run_infoshare_L2_V2_random_flip1(mags):
    ...
    for m in mags:
        params = {}
        if m == 8:
            params={"NUM_ITERATIONS":10,"OUTPUT_DIR":'../results/infoshare_flip_'+str(num_flips)+'_results_L2',"MAGNITUDE":m,"V":1,"T":50,"WINDOW_LENGTH":3,"ALGORITHM":"INFO_SHARE","SIM_NUMBER":110,"NUM_FLIPS":num_flips}
        else:
            params={"NUM_ITERATIONS":10,"OUTPUT_DIR":'../results/infoshare_flip_'+str(num_flips)+'_results_L2',"MAGNITUDE":m,"V":1,"T":50,"WINDOW_LENGTH":3,"ALGORITHM":"INFO_SHARE","SIM_NUMBER":446,"NUM_FLIPS":num_flips}
...
def run_infoshare_L2_V2_random_flip(mags):
    ...
    for m in mags:
        params = {}
        if m == 8:
            params={"NUM_ITERATIONS":10,"OUTPUT_DIR":'../results/infoshare_flip_'+str(prob)+'_results_L2',"MAGNITUDE":m,"V":1,"T":50,"WINDOW_LENGTH":3,"ALGORITHM":"INFO_SHARE","SIM_NUMBER":110}
        else:
            params={"NUM_ITERATIONS":10,"OUTPUT_DIR":'../results/infoshare_flip_'+str(prob)+'_results_L2',"MAGNITUDE":m,"V":1,"T":50,"WINDOW_LENGTH":3,"ALGORITHM":"INFO_SHARE","SIM_NUMBER":446}
