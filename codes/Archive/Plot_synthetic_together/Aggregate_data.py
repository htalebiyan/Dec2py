...
lambda_df_old = lambda_df_old.replace({'Uniform': 'UNIFORM', 'judgeCall_OPTIMISTIC': 'jc',
                                        '': 'nan'})
lambda_df_old = lambda_df_old.rename({'lambda_TC': 'lambda_tc', 'lambda_P': 'lambda_p'}, axis='columns')
...
# [lambda_df_old, alloc_df_old, time_df_old] = pd.read_pickle('postprocess_dicts_all_topo.pkl')
...
lambda_df_new2 = pd.DataFrame()
alloc_df_new2 = pd.DataFrame()
time_df_new2 = pd.DataFrame()
for to in Topo:
    ...
    rslt = pd.read_pickle(results_dir+to+'\postprocess_sample_5_9\postprocess_dicts.pkl')
    ...
    results_dir = "C:\\Users\\ht20\\Documents\\Files\\Auction_synthetic_networks_v3.1\\"
    rslt = pd.read_pickle(results_dir+to+'\postprocess_sample_10_14\postprocess_dicts.pkl')
    rslt[4] = rslt[4].assign(topology=to)
    lambda_df_new2 = pd.concat([lambda_df_new2, rslt[4]])
    rslt[6] = rslt[6].assign(topology=to)
    alloc_df_new2 = pd.concat([alloc_df_new2, rslt[6]])
    rslt[7] = rslt[7].assign(topology=to)
    time_df_new2 = pd.concat([time_df_new2, rslt[7]])

LAMBDA_DF = pd.concat([lambda_df_old, lambda_df_new, lambda_df_new2])
...
ALLOC_GAP_DF = pd.concat([alloc_df_old, alloc_df_new, alloc_df_new2])
ALLOC_GAP_DF=pd.merge(ALLOC_GAP_DF, config_info,
              left_on=['Magnitude','topology'],
              right_on=['Config Number','topology']) 
ALLOC_GAP_DF = ALLOC_GAP_DF.reset_index()

RUN_TIME_DF = pd.concat([time_df_old, time_df_new, time_df_new2])
